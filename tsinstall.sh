...
if [ -d "/root/ts" ] ; then
	echo "The folder /root/ts already exists, please rename or delete it before running the script."
...
# --------------------------------------------------
# twinsaga - 012.001.01.49
# --------------------------------------------------
if [ "$TSVERSION" = 1 ] ; then
	cd "/root/ts"
	wget --no-check-certificate "https://raw.githubusercontent.com/hookarik/aktest/master/twinsaga_012_001_01_49" -O "twinsaga_012_001_01_49"
	chmod 777 twinsaga_012_001_01_49
	. "/root/ts/twinsaga_012_001_01_49"
	...
	sed -i "s/\x44\x24\x0c\x28\x62\x34/\x44\x24\x0c\x08\x49\x40/g" "MissionServer/MissionServerTS"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "WorldServer/WorldServerTS"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "ZoneServer/ZoneServerTS"
	...
	sudo -u postgres psql -d ARAccount -c "\i '/root/ts/SQL/ARAccount.sql';"
	sudo -u postgres psql -d ARDB1 -c "\i '/root/ts/SQL/ARDB1.sql';"
	sudo -u postgres psql -d ARMember -c "\i '/root/ts/SQL/ARMember.sql';"
	...
	rm -f twinsaga_012_001_01_49
	
	# setup info
	VERSIONNAME="twinsaga - 012.001.01.49"
	CREDITS="X-Legends"
...
if [ "$VERSIONNAME" = "NONE" ] ; then
	...
	echo -e "\nTo start the server, please run /root/ts/start"
	echo -e "To stop the server, please run /root/ts/stop${RC}"
