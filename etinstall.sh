...
if [ -d "/root/et" ] ; then
	echo "The folder /root/et already exists, please rename or delete it before running the script."
...
# --------------------------------------------------
# edeneternal - 002.003.05.67
# --------------------------------------------------
if [ "$DSVERSION" = 1 ] ; then
	cd "/root/et"
	wget --no-check-certificate "https://raw.githubusercontent.com/hookarik/aktest/master/edeneternal_002_003_05_67" -O "edeneternal_002_003_05_67"
	chmod 777 edeneternal_002_003_05_67
	. "/root/et/edeneternal_002_003_05_67"
	...
	sed -i "s/\x44\x24\x0c\x28\x62\x34/\x44\x24\x0c\x08\x49\x40/g" "MissionServer/MissionServerET"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "WorldServer/WorldServerET"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "ZoneServer/ZoneServerET"
	...
	sudo -u postgres psql -c "create database \"ETAccount\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"ETDB1\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"ETMember\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"ETMember1\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -d ETAccount -c "\i '/root/et/SQL/ETAccount.sql';"
	sudo -u postgres psql -d ETDB1 -c "\i '/root/et/SQL/ETDB1.sql';"
	sudo -u postgres psql -d ETMember -c "\i '/root/et/SQL/ETMember.sql';"
	sudo -u postgres psql -d ETMember1 -c "\i '/root/et/SQL/ETMember1.sql';"
	sudo -u postgres psql -d ETAccount -c "UPDATE worlds SET ip = '$IP' WHERE ip = '192.168.178.59';"
	sudo -u postgres psql -d ETDB1 -c "UPDATE serverstatus SET ext_address = '$IP' WHERE ext_address = '192.168.178.59';"
	sudo -u postgres psql -d ETDB1 -c "UPDATE serverstatus SET int_address = '$IP' WHERE int_address = '192.168.178.59';"
	
	# remove server setup files
	rm -f edeneternal_002_003_05_67
	
	# setup info
	VERSIONNAME="edeneternal - 002.003.05.67"
	CREDITS="X-Legends"
...
if [ "$VERSIONNAME" = "NONE" ] ; then
	...
	echo -e "\nTo start the server, please run /root/et/start"
	echo -e "To stop the server, please run /root/et/stop${RC}"
