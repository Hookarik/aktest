...
if [ -d "/root/ee" ] ; then
	echo "The folder /root/ee already exists, please rename or delete it before running the script."
...
if [ "$DSVERSION" = 1 ] ; then
	...
	. "/root/ee/edeneternal_002_003_05_67"
	...
	sudo -u postgres psql -c "create database \"EEAccount\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"EEDB1\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"EEMember\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"EEMember1\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -d EEAccount -c "\i '/root/ee/SQL/EEAccount.sql';"
	sudo -u postgres psql -d EEDB1 -c "\i '/root/ee/SQL/EEDB1.sql';"
	sudo -u postgres psql -d EEMember -c "\i '/root/ee/SQL/EEMember.sql';"
	sudo -u postgres psql -d EEMember1 -c "\i '/root/ee/SQL/EEMember1.sql';"
	sudo -u postgres psql -d EEAccount -c "UPDATE worlds SET ip = '$IP' WHERE ip = '192.168.178.59';"
	sudo -u postgres psql -d EEDB1 -c "UPDATE serverstatus SET ext_address = '$IP' WHERE ext_address = '192.168.178.59';"
	sudo -u postgres psql -d EEDB1 -c "UPDATE serverstatus SET int_address = '$IP' WHERE int_address = '192.168.178.59';"
...
if [ "$VERSIONNAME" = "NONE" ] ; then
	...
	echo -e "\nTo start the server, please run /root/ee/start"
	echo -e "To stop the server, please run /root/ee/stop${RC}"
