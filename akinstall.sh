...
if [ -d "/root/ak" ] ; then
	echo "The folder /root/ak already exists, please rename or delete it before running the script."
...
if [ "$AKVERSION" = 1 ] ; then
	cd "/root/ak"
	wget --no-check-certificate "https://raw.githubusercontent.com/hookarik/aktest/master/xiaoguai475_015_001_01_16" -O "xiaoguai475_015_001_01_16"
	chmod 777 xiaoguai475_015_001_01_16
	. "/root/ak/xiaoguai475_015_001_01_16"
	...
	sudo -u postgres psql -d FFAccount -c "\i '/root/ak/SQL/FFAccount.bak';"
	sudo -u postgres psql -d FFDB1 -c "\i '/root/ak/SQL/FFDB1.bak';"
	sudo -u postgres psql -d FFMember -c "\i '/root/ak/SQL/FFMember.bak';"
...
# --------------------------------------------------
# aurakingdom - 015.001.01.16
# --------------------------------------------------
if [ "$AKVERSION" = 2 ] ; then
	cd "/root/ak"
	wget --no-check-certificate "https://raw.githubusercontent.com/hookarik/aktest/master/aurakingdom_015_001_01_16" -O "aurakingdom_015_001_01_16"
	chmod 777 aurakingdom_015_001_01_16
	. "/root/ak/aurakingdom_015_001_01_16"
	...
	rm -f "server.zip"
	sed -i "s/xxxxxxxx/$DBPASS/g" "GatewayServer/setup.ini"
	sed -i "s/\x44\x24\x0c\x28\x62\x34/\x44\x24\x0c\x08\x49\x40/g" "MissionServer/MissionServerAK"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "WorldServer/WorldServerAK"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "ZoneServer/ZoneServerAK"
	...
	# remove server setup files
	rm -f aurakingdom_015_001_01_16
	
	# setup info
	VERSIONNAME="aurakingdom - 015.001.01.16"
	CREDITS="X-Legends"
	THREADLINK="https://forum.ragezone.com/threads/aura-kingdom-released-files.1204666/"
fi
...
if [ "$VERSIONNAME" = "NONE" ] ; then
	...
	echo -e "\nTo start the server, please run /root/ak/start"
	echo -e "To stop the server, please run /root/ak/stop${RC}"
