...
if [ -d "/root/ds" ] ; then
	echo "The folder /root/ds already exists, please rename or delete it before running the script."
...
# --------------------------------------------------
# dragonslayer - 001.001.01.93
# --------------------------------------------------
if [ "$DSVERSION" = 1 ] ; then
	cd "/root/ds"
	wget --no-check-certificate "https://raw.githubusercontent.com/hookarik/aktest/master/dragonslayer_001_001_01_93" -O "dragonslayer_001_001_01_93"
	chmod 777 dragonslayer_001_001_01_93
	. "/root/ds/dragonslayer_001_001_01_93"
	...
	sed -i "s/\x44\x24\x0c\x28\x62\x34/\x44\x24\x0c\x08\x49\x40/g" "MissionServer/MissionServerDS"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "WorldServer/WorldServerDS"
	sed -i "s/\x3d\xc0\xa8\xb2/\x3d$PATCHIP/g" "ZoneServer/ZoneServerDS"
	...
	sudo -u postgres psql -c "create database \"DSAccount\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"DSDB1\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -c "create database \"DSMember\" encoding 'SQL_ASCII' template template0;"
	sudo -u postgres psql -d DSAccount -c "\i '/root/ds/SQL/DSAccount.sql';"
	sudo -u postgres psql -d DSAccount -c "\i '/root/ds/SQL/fortune_bag.sql';"
	sudo -u postgres psql -d DSDB1 -c "\i '/root/ds/SQL/DSDB1.sql';"
	sudo -u postgres psql -d DSMember -c "\i '/root/ds/SQL/DSMember.sql';"
	sudo -u postgres psql -d DSAccount -c "UPDATE worlds SET ip = '$IP' WHERE ip = '192.168.178.59';"
	sudo -u postgres psql -d DSDB1 -c "UPDATE serverstatus SET ext_address = '$IP' WHERE ext_address = '192.168.178.59';"
	sudo -u postgres psql -d DSDB1 -c "UPDATE serverstatus SET int_address = '$IP' WHERE int_address = '192.168.178.59';"
	
	# remove server setup files
	rm -f dragonslayer_001_001_01_93
	
	# setup info
	VERSIONNAME="dragonslayer - 001.001.01.93"
...
if [ "$VERSIONNAME" = "NONE" ] ; then
	...
	echo -e "\nTo start the server, please run /root/ds/start"
	echo -e "To stop the server, please run /root/ds/stop${RC}"
